<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="score" width="60px" label="分数">
        <template slot-scope="{row}">
          <span>{{ row.score }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="rank" width="60px" label="名次">
        <template slot-scope="{row}">
          <span>{{ row.rank }}</span>
        </template>
      </el-table-column>
    ...
    <!-- 分页 -->
    <pagination v-show="total>0 && isShow" class="center" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />
    <!-- 弹出框 -->
  </div>
</template>

<script>
import { Audit, Return } from '@/api/Competition/works/unrise'
