<template>
  <div>
    ...
    >
      ...
      <el-table-column v-if="operability" label="操作" align="center" width="200px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" @click.stop="handleDeleteData(row)">
            删除过程数据
          </el-button>
        </template>
      </el-table-column>
...
import { getSortCompetitions, deleteData } from '@/api/Competition/sort/CompetitonSorted'
...
export default {
  ...
  methods: {
    ...
    getList() {
      getSortCompetitions(this.listQuery).then(response => {
    ...
    handleDeleteData(row) {
      this.$confirm('此操作会删除评委评分相关数据且不可恢复', '提示', {
