  ...
  <div>
    ...
    >
      ...
      <el-table-column :class="[tagbutton]" label="竞赛名称" min-width="200px" align="center">
        <template slot-scope="{row}">
          <el-tag :class="[tagbutton]" type="info">{{ row.match_project }}</el-tag>
          <span>{{ row.match_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="作品名称" align="center" min-width="200px">
        <template slot-scope="{row}">
          <el-tag :class="[tagbutton]" type="info">{{ row.type }}</el-tag>
      ...
      <el-table-column label="作品资源" align="center" width="180px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="info" size="small" icon="el-icon-download" @click.stop="handleDownloadOne(row)">
            下载
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="isCourseAdmin" label="操作" align="center" width="180" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button v-if="row.normalization === 5" :class="[jsqxbutton]" size="small" disabled @click="handleFin(row,$index)">
            直接展示
          </el-button>
